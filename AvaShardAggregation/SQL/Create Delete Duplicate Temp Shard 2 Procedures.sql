-- Document
CREATE OR ALTER PROCEDURE sp_Delete_Duplicate_Document_with_Temp_Shard_2
AS
BEGIN
	...
	(
		SELECT d.DocumentId 
		FROM Document d
		INNER JOIN Document_Temp_Shard_2 dts1
...
-- DocumentLine
CREATE OR ALTER PROCEDURE sp_Delete_Duplicate_DocumentLine_with_Temp_Shard_2
AS
BEGIN
	...
	(
		...
		INNER JOIN DocumentLine_Temp_Shard_2 dlts1
...
-- BoundaryOverrideId
CREATE OR ALTER PROCEDURE sp_Delete_Duplicate_BoundaryOverride_with_Temp_Shard_2
AS
BEGIN
	...
	(
		...
		INNER JOIN BoundaryOverride_Temp_Shard_2 bots1
...
CREATE OR ALTER PROCEDURE sp_Delete_Duplicate_JurisdictionOverride_with_Temp_Shard_2
AS
BEGIN
	...
	(
		...
		INNER JOIN JurisdictionOverride_Temp_Shard_2 jots1
...
-- Service
CREATE OR ALTER PROCEDURE sp_Delete_Duplicate_Service_with_Temp_Shard_2
AS 
BEGIN 
	...
	(
		SELECT s.ServiceId 
		FROM Service s
		INNER JOIN Service_Temp_Shard_2 sts1
...
-- Subscription
CREATE OR ALTER PROCEDURE sp_Delete_Duplicate_Subscription_with_Temp_Shard_2
AS 
BEGIN 
	...
	(
		...
		FROM Subscription s
		INNER JOIN Subscription_Temp_Shard_2 sts1
		ON s.AccountId = sts1.AccountId  
...
-- User
CREATE OR ALTER PROCEDURE sp_Delete_Duplicate_User_with_Temp_Shard_2
AS 
BEGIN 
	DELETE FROM [User]	
	WHERE UserId IN 
	(
		SELECT u.UserId 
		FROM [User] u 
		INNER JOIN User_Temp_Shard_2 uts1 
...
-- Company
CREATE OR ALTER PROCEDURE sp_Delete_Duplicate_Company_with_Temp_Shard_2
AS 
BEGIN 
	DELETE FROM Company
	WHERE CompanyId IN
	(
		SELECT c.CompanyId 
		FROM Company c
		INNER JOIN Company_Temp_Shard_2 cts1
...
-- TaxRule
CREATE OR ALTER PROCEDURE sp_Delete_Duplicate_TaxRule_with_Temp_Shard_2
AS 
BEGIN 
	...
	(
		SELECT tr.TaxRuleId 
		FROM TaxRule tr
		INNER JOIN TaxRule_Temp_Shard_2 trts1
...
--CompanyLocation
CREATE OR ALTER PROCEDURE sp_Delete_Duplicate_CompanyLocation_with_Temp_Shard_2
AS 
BEGIN 
	...
	(
		...
		INNER JOIN CompanyLocation_Temp_Shard_2 clts1
...
--Nexus
CREATE OR ALTER PROCEDURE sp_Delete_Duplicate_Nexus_with_Temp_Shard_2
AS 
BEGIN 
	...
	(
		SELECT n.NexusId 
		FROM Nexus n
		INNER JOIN Nexus_Temp_Shard_2 nts1
...
-- TaxCode
CREATE OR ALTER PROCEDURE sp_Delete_Duplicate_TaxCode_with_Temp_Shard_2
AS 
BEGIN 
	...
	(
		SELECT tc.TaxCodeId 
		FROM TaxCode tc
		INNER JOIN TaxCode_Temp_Shard_2 tcts1
...
CREATE OR ALTER PROCEDURE sp_Delete_Duplicate_AvaCertServiceConfig_with_Temp_Shard_2
AS 
BEGIN 
	...
	(
		...
		INNER JOIN AvaCertServiceConfig_Temp_Shard_2 acscts1
...
--CompanyReturn
CREATE OR ALTER PROCEDURE sp_Delete_Duplicate_CompanyReturn_with_Temp_Shard_2
AS 
BEGIN 
	...
	(
		...
		INNER JOIN CompanyReturn_Temp_Shard_2 crts1
...
-- Item
CREATE OR ALTER PROCEDURE sp_Delete_Duplicate_Item_with_Temp_Shard_2
AS 
BEGIN 
	...
	(
		SELECT i.ItemId 
		FROM Item i
		INNER JOIN Item_Temp_Shard_2 its1
