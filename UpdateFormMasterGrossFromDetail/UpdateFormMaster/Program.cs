...
namespace UpdateFormMaster
{
    class Program
    {
        ...
        {
            List<InputData> FormGrossFromDetails = new List<InputData>(); // = null;
            ...
            using (FileStream fs = new FileStream(args[0], FileMode.Open, FileAccess.Read, FileShare.ReadWrite))
            {
                ...
                using (SpreadsheetDocument spreadsheetDocument = SpreadsheetDocument.Open(fs, false))
                {
                    ...
                    {
                        rowNumber++;
                        InputData FormGrossFromDetail = new InputData();
                        int rowcolumn = 1;
                        if (rowNumber > 1)
                        {
                            foreach (Cell c in row.Elements<Cell>())
                            {
                                switch (rowcolumn)
                                {
                                    case 1: FormGrossFromDetail.TaxFormCode = ReadExcelCell(c, workbookPart); break;
                                }

                                rowcolumn++;
                            }

                            FormGrossFromDetails.Add(FormGrossFromDetail);
                        }
                    }
                }
            }

            int totalRecords = FormGrossFromDetails.Count;
            int recordsUpdated = 0;
            int recordsErrored = 0;
            
            foreach (InputData item in FormGrossFromDetails)
            {
                ...
                else
                {
                    try
                    {
                        currentFormMaster.GrossFromDetail = true;
            ...
            Console.WriteLine(string.Format("5: Main: {0} records updated and {1} records errored out of {2} total records.", recordsUpdated.ToString(), recordsErrored.ToString(), totalRecords.ToString()));
            Console.ReadKey();
            ...
            try
            {
                ...
                using (var response = (HttpWebResponse)request.GetResponse())
                {
                    if (response != null)
                    {
                        ...
                        {
                            Console.WriteLine("8: UpdateForMMaster: Unable to update FormMaster for TaxFormCode [{0}] due to an unhandled exception:[{1}]", updatedFormMaster.TaxFormCode, responseString);
                            return false;
                        }
