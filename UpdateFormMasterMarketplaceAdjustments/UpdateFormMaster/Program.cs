...
namespace UpdateFormMaster
{
    class Program
    {
        ...
        {
            List<InputData> FormMasterMarketPlaces = new List<InputData>(); // = null;
            ...
            using (FileStream fs = new FileStream(args[0], FileMode.Open, FileAccess.Read, FileShare.ReadWrite))
            {
                ...
                using (SpreadsheetDocument spreadsheetDocument = SpreadsheetDocument.Open(fs, false))
                {
                    ...
                    {
                        rowNumber++;
                        InputData FormMasterMarketPlace = new InputData();
                        int rowcolumn = 1;
                        if (rowNumber > 1)
                        {
                            foreach (Cell c in row.Elements<Cell>())
                            {
                                switch (rowcolumn)
                                {
                                    case 1: FormMasterMarketPlace.TaxFormCode = ReadExcelCell(c, workbookPart); break;
                                    case 2: FormMasterMarketPlace.LegacyReturnName = ReadExcelCell(c, workbookPart); break;
                                    case 3: FormMasterMarketPlace.SplitOutOfStateLocations = Convert.ToBoolean(Convert.ToInt16(ReadExcelCell(c, workbookPart))); break;
                                    case 4: FormMasterMarketPlace.MarketplaceTransactionsAsDeductions = Convert.ToBoolean(Convert.ToInt16(ReadExcelCell(c, workbookPart))); break;
                                }

                                rowcolumn++;
                            }

                            FormMasterMarketPlaces.Add(FormMasterMarketPlace);
                        }
                    }
                }
            }

            int totalRecords = FormMasterMarketPlaces.Count;
            int recordsUpdated = 0;
            int recordsErrored = 0;

            foreach (InputData item in FormMasterMarketPlaces)
            {
                ...
                else
                {
                    try
                    {
                        currentFormMaster.SplitOutOfStateLocations = item.SplitOutOfStateLocations;
                        currentFormMaster.MarketplaceTransactionsAsDeductions = item.MarketplaceTransactionsAsDeductions;
